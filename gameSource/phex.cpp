...
void Phex::chatCmdHELP(std::vector<std::string> input) {
	for (std::pair<std::string, ChatCommand> element : chatCommands) {
		...
		if (strEquals(element.first, "test") || (strEquals(element.first, "life") && !(HetuwMod::FakeUID != 0))) continue;
...
void Phex::joinChannel(std::string inChannelName) {
	...
	sendServerLife(HetuwMod::FakeUID != 0 ? HetuwMod::FakeUID : HetuwMod::ourLiveObject->id);
